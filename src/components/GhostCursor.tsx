...
const GhostCursor = ({
  ...
  const fragmentShader = `
    ...
    float fbm(vec2 p){
      float v=0.,a=0.5; mat2 m=mat2(cos(0.5),sin(0.5),-sin(0.5),cos(0.5));
      for(int i=0;i<5;i++){v+=a*noise(p);p=m*p*2.;a*=0.5;} return v;
    }
  ...
  return <div ref={containerRef} className={`absolute inset-0 pointer-events-none ${className ?? ""}`} style={mergedStyle} />;
};
